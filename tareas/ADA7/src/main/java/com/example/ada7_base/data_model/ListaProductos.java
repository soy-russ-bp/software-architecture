...
public class ListaProductos {
    ...
    public void votarProducto(String nombre) {
        ...
        for (Producto producto : this.productos) {
            if (nombre.equals(producto.getNombre())) {
                ...
                try {
                    producto.addVotosArchivoPropio(); // Agregar voto al archivo
    ...
    private Producto getProducto(int numlinea) {
        String[] datos = new String[3]; // Arreglo para nombre, votos, imagen
        StringBuilder sb = new StringBuilder();
        int currentLine = 0;

        try (BufferedReader br = new BufferedReader(new FileReader("src/main/java/com/example/ada7_base/data_model/productos.txt"))) {
            String linea;
